...
class AmazonDomeSpiderMiddleware(object):
...
class AmazonDomeDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
...
from amazon_dome.settings import USER_AGENT_LIST
...
from amazon_dome.settings import IP_PROXY_LIST
...
class SeleniumMiddleware:
    def process_request(self, request, spider):
        ...
        elif spider.name =='book_spider' and request.meta.get('type', None):
            spider.driver.get(request.url)
            time.sleep(3)
            # 中国历史
            spider.driver.find_element_by_xpath(
                f"//div/ul[1]/li[1]/div/div/dl[1]/dt/a").click()
            # button_list = []
            # for j in range(1, 9):
